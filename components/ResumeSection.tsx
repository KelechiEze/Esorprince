  ...
  return (
    <section id="resume" className="lg:min-h-screen flex flex-col justify-center py-10 lg:py-24 max-w-2xl mx-auto overflow-hidden">
      <div data-gsap-animate="fade-right" className="flex items-center gap-2 border border-[#333] rounded-full px-4 py-1.5 w-fit mb-6 lg:mb-10">
        ...
        <span className="text-[9px] uppercase tracking-[2px] font-medium text-white">Resume</span>
      </div>
      ...
      <div data-gsap-stagger className="relative border-l border-[#333] ml-2 md:ml-4 pl-8 md:pl-10 space-y-10 lg:space-y-14">
        {experiences.map((group, idx) => (
          <div key={idx} className="relative group">
            <div className="absolute -left-[37px] md:-left-[51px] top-1 w-2.5 h-2.5 rounded-full bg-[#666] group-hover:bg-primary transition-colors duration-300"></div>
            
            <div className="text-[#999] text-sm mb-3 lg:mb-5 group-hover:text-primary transition-colors duration-300">
              {group.period}
            </div>
            ...
            <div className="space-y-6 lg:space-y-8">
              {group.items.map((item, itemIdx) => (
                <div key={itemIdx} className="space-y-1.5">
                  <h3 className="text-xl md:text-2xl font-light hover:text-primary transition-colors duration-300 cursor-default">
                    {item.title}
                  </h3>
                  <p className="text-[#666] text-xs uppercase tracking-wide">
                    {item.company}
                  </p>
                </div>
              ))}
